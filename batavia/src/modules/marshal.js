/*************************************************************************
 * Marshal
 * This marshal contains functions that can read and write Python values in
 * a binary format. The format is specific to Python, but independent of
 * machine architecture issues.

 * Not all Python object types are supported; in general, only objects
 * whose value is independent from a particular invocation of Python can be
 * written and read by this marshal. The following types are supported:
 ...
 * Variables:
 *
 * version -- indicates the format that the marshal uses. Version 0 is the
...
// } while(0)
    ...
    r_object: function (vm, p) {
        /* null is a valid return value, it does not necessarily means that
         an exception is set. */
        let retval, v, buf, idx = 0, i, n, type, code = marshal.r_byte(vm, p), flag;
        ...
        switch (type) {
            ...
            case marshal.TYPE_CODE:
                ...
                v = new Code({
                    ...
                    stacksize: stacksize,
                    flags: flags,
                    code: code.split('').map(function (b) {
                        return b.charCodeAt();
                    }),
                    consts: consts,
                    names: names,
