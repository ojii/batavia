...
export const inspect = {
    ...
    _signature_bound_method: function (sig) {
        ...
        if (!params || [inspect.VAR_KEYWORD, inspect.KEYWORD_ONLY].indexOf(params[0].kind) !== -1) {
            throw new builtins.ValueError('invalid method signature');
        }

        var kind = params[0].kind;
        if ([inspect.POSITIONAL_OR_KEYWORD, inspect.POSITIONAL_ONLY].indexOf(kind) !== -1) {
            // Drop first parameter:
            // '(p1, p2[, ...])' -> '(p2[, ...])'
            params = params.slice(1);
        } else {
            if (kind !== inspect.VAR_POSITIONAL) {
                // Unless we add a new parameter type we never
                // get here
                throw new builtins.ValueError('invalid argument type');
            }
            // It's a var-positional parameter.
            // Do nothing. '(*args[, ...])' -> '(*args[, ...])'
        }

        return new inspect.Signature(params, sig.return_annotation);
    },

    _signature_internal: function (obj) {
        // if (!callable(obj)) {
    ...
    getcallargs: function (func, positional, named) {
        ...
        for (var kw in named) {
            if (named.hasOwnProperty(kw)) {
                if (!(kw in possible_kwargs)) {
                    if (!func.argspec.varkw) {
                        throw new builtins.TypeError(
                            `${func.__name__} got an unexpected keyword argument ${builtins.repr(kw)}`
                        );
                    }
                ...
                if (kw in arg2value) {
                    throw new builtins.TypeError(
                        `${func.__name__} got multiple values for argument ${builtins.repr(kw)}`
                    );
                }
        ...
        if (num_pos > num_args && func.argspec.varargs.length === 0) {
...
inspect.Parameter = class {
    ...
    replace(kwargs) {
        const name = kwargs.name || this.name,
            kind = kwargs.kind || this.kind,
            annotation = kwargs.annotation || this.annotation,
            def = kwargs.default || this.default;

        return new inspect.Parameter(name, kind, def, annotation);
    }
};
...
inspect.Signature = class {
    constructor(parameters, return_annotation, __validate_parameters__) {
        this.parameters = {};
        if (parameters !== null) {
